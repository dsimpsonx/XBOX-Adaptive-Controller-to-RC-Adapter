...
void loop() {
  ...
  if (Xbox.XboxOneConnected) {
    ...
    if (Xbox.getButtonClick(L3) || Xbox.getButtonClick(RIGHT) || Xbox.getButtonClick(R1)){ // Incrementally trim steering to turn right
      steeringTrim = steeringTrim + 5;
      steeringTrimRefreshLCD();
    }
    ...
    if (Xbox.getAnalogHat(LeftHatY)) {
      if (Xbox.getAnalogHat(LeftHatY) > 2500 || Xbox.getAnalogHat(LeftHatY) < -2500) {
        ppm[1] = map(Xbox.getAnalogHat(LeftHatY), -32768 , 32768, 1000, 2000);
      } else {
        ppm[1] = 1500;
      }
    } 
